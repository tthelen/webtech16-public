...
class App:
    ...
    def add_route(self, partial_route, handler, methods=None):
        """
        ...
        :param methods: List of allowed methods. If methods is None, all methods are allowed.
        ...
        if not prefixed_route.endswith('$'):  # add end anchor if necessary
            prefixed_route += '$'
        return self.server.add_route(prefixed_route, handler, methods)
...
class Request:
    ...
    def __init__(self):
        ...
        self.jsondata = None  # for parsed json payload
...
class Response:
    ...
    def send_json(self, code=None, headers=None, data=""):
...
class Webserver:
    ...
    def add_route(self, route, action, methods=None):
        """
        Register a route for request processing.
        """
        self.routes.append((route, action, methods))
