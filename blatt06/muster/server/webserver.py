...
class Response:
    ...
    def __init__(self, conn, server):
        ...
        self.headers['Content-Type'] = "text/html; charset=UTF-8" # default
        self.cookies = []
        self.body = None  # response body
        self.server = server
        self.code = 200 # default
    ...
    def serve(self):
        ...
        while 1:
            ...
            if self.request.parse(conn):
                ...
                # preprocessing (middlewares)
                # TT: blatt6 - moved to position right before comitting the response
                # TT: because only then all the information needed for logging ist available
                # TT: By doing so, middlewares are no longer able to raise StopProcessing
                # TT: exceptions when processing the response. This is probably no problem.
                for m in self.middlewares:
