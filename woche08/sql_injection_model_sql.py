...
The model defines two classes: Persons and Teams.
 ...
 \ \        / /\   |  __ \| \ | |_   _| \ | |/ ____|
  ...
  VARIANT WITH SQL INJECTION VULNERABILITIES
...
class Person():
    ...
    def find(cls, **kwargs):
        ...
        for k,v in kwargs.items():
            sql += "AND {} = '{}' ".format(k, v)  # WARNING! SQL INJECTION VULNERABILITY

        print(sql)
        persons = [Person(dbobj=row) for row in conn.execute(sql)]  # WARNING! SQL INJECTION VULNERABILITY
        conn.commit()
        return persons
