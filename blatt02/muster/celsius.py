...
class CelsiusApp(webserver.App):
    ...
    def celsius(self, request, response, pathmatch=None):
        msg = ""
        if 'celsius' in request.params:  # check if celsius parameter is given
            try:  # calculate
        ...
        if 'fahrenheit' in request.params:
            try:  # calculate
                celsius = (float(request.params['fahrenheit']) - 32) * 5/9
                msg = "{}° Fahrenheit sind {:4.2f}° Celsius".format(request.params['fahrenheit'], celsius)
            except (ValueError, TypeError):
                msg = "Bitte eine Zahl eingeben."
