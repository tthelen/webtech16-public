...
class SessionMiddleware(Middleware):
    ...
    def process_request(self, request, response):
        ...
        if self.cookiename in request.params:  # try to recover old session
            self.session = Session(request.params[self.cookiename], cookiename=self.cookiename)
            log(2,"Loaded from Session: %s" % self.session.data)
        elif self.cookiename in request.cookies:  # try to recover old session
